...
- `src/audio.rs` — `PulseManager`, the routing core. Talks to PipeWire via the PulseAudio API. Creates two null-sink buses (`OpenWave_Monitor`, `OpenWave_Stream`), routes each channel through a pair of `module-loopback`s (one per mix — per-channel/per-mix volume+mute are plain sink-input operations on those loopbacks), exposes the stream bus as a capture device via `module-remap-source`, and drives level meters with low-rate peek streams. Emits `AudioEvent`s consumed by the UI. Self-healing: re-applies volumes and re-attaches streams if the session manager moves them; cleans up stale `OpenWave_*` modules from crashed sessions on startup.
